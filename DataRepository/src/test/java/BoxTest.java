...
public class BoxTest {
    ...
    public void boxInsertSpeedTest() {
        ...
        System.out.println(box.take(1, field[0], field[1]).toString());

        try {
            box.storage(Paths.get("F:\\xz\\tool\\javaback\\xx.dat"));
        } catch (IOException e) {
            throw new RuntimeException(e);
        }

        try {
            Box loadBox = BoxFactory.load(Paths.get("F:\\xz\\tool\\javaback\\xx.dat"));
            JSONArray take = loadBox.take(-1);
